...
export const MintSubnameForm = ({
  ...
  const sdkListing: Listing = {
    fullName: listing.fullName,
    label: listing.label,
    network: "mainnet",
    node: namehash(listing.fullName),
    listingType: "l2",
  ...
  const checkAvailable = async (subnameLabel: string) => {
    ...
    let available = false;
    try {
      available = await namespaceClient.isSubnameAvailable(sdkListing, subnameLabel);
    } catch(err) {
  ...
  if (chain && chain.id !== base.id) {
    return <ChangeMintNetwork requiredNetwork="base"></ChangeMintNetwork>
  }
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      const params = await namespaceClient.getMintTransactionParameters(sdkListing, {
      ...
      await publicClient?.waitForTransactionReceipt({hash: tx as Hash, confirmations: 2});
